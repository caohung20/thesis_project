...
Qsh=busdata['Qshunt[kvar]'].values
...
while maxerror >= accuracy and iter <= maxiter:
    ...
    A = np.zeros((m,m))     #initializing Jacobian matrix
    iter+=1
    for n in range(1, nbus+1):
        ...
        if kb[n-1] != 3:
            ...
            DC[nn-1] = P[n-1] - Pk
        if kb[n-1] == 1:
            A[nn-1][lm-1] = 2 * Vm[n-1] * Ym[n-1][n-1] * math.cos(t[n-1][n-1]) + J22    #diagonal elements of J2
            #diagonal elements of J3
            A[lm-1][nn-1] = J33         
            A[lm-1][lm-1] = -2 * Vm[n-1] * Ym[n-1][n-1] * math.sin(t[n-1][n-1]) - J44   #diagonal elements of J4
            DC[lm-1] = Q[n-1] - Qk
